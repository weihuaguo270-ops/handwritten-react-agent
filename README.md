...
│   │   │       └── replay.py
│   │   └── __init__.py
│   └── eval-engine/     # 自适应 Process Reward 评估引擎（实验三）
│       ├── core/              # 手写核心
...
│       │   ├── permissions.py      # 四级权限控制
│       │   └── human_in_the_loop.py# 人工审批管理器
│       ├── intent/            # 意图分类
│       │   └── classifier.py
│       ├── judge/             # Judge LLM 系统
│       │   ├── executor.py
...
│       ├── dataset/           # 测试数据集
│       └── tests/
│   └── trace-debugger/    # 执行轨迹复盘分析（实验四）
│       ├── trace_debugger/
│       │   ├── reader.py        # 轨迹 JSON 解析
│       │   ├── analyzer.py      # 路径分析 + 失败分类
│       │   ├── reporter.py      # 复盘报告生成
│       │   └── cli.py           # CLI 入口
│       ├── README.md
│       └── pyproject.toml
...
## 实验三：Eval Engine — 自适应 Process Reward 评估

`experiments/eval-engine/` 是 **生产级的 Agent 质量评估引擎**，不依赖任何外部框架，全部手写。

### 核心创新

| 创新点 | 说明 |
|--------|------|
| **动态评分标准生成** | 不是固定模板，针对每步上下文动态生成评分标准 |
| **Process Reward 评分** | 步骤级评分（不只看最终答案），受 o1/o3 启发 |
| **自适应 Eval Loop** | 低分步骤 → 自动修正指令 → LLM 重试 → 再评分 |
| **四级权限控制** | SAFE/NOTIFY/CONFIRM/DENY，高风险操作需用户确认 |
| **意图自动路由** | 测试类→直接返回，生成类→进入 Eval Loop |

### 架构

```
用户输入 → IntentClassifier → 测试类 → Agent 执行 → 返回用户
                            → 生成类 → Agent 执行 → Process Reward → 全部达标 → 返回
                                                              → 有低分 → 问用户 → 修正 → 重试
                                                                                     │
                                                                         工具调用前检查权限 ← PermissionWrapper
```

### 与手写版的关系

```
src/handwritten_react_agent/eval/     experiments/eval-engine/
  ├─ 静态关键词匹配                     ├─ 语义级 LLM-as-Judge
  ├─ 4 维评分（工具/内容/步数/答案）      ├─ 12 维 Process Reward
  └─ 离线报告                           ├─ 自适应 Eval Loop
                                       ├─ CI/CD 回归门禁
                                       ├─ 四级权限控制
                                       └─ Human-in-the-Loop
...
```python
from experiments.eval-engine.integration.agent_wrapper import create_guarded_agent

guarded = create_guarded_agent(
    agent_fn=react_loop,
    execute_tool_fn=execute_tool_call,
    ask_fn=ask_user_for_approval,
    enable_permissions=True,   # 工具调用权限拦截
    enable_auto_eval=True,     # 自动质量评估
)
result = guarded("帮我写一份报告")  # 自动评估+权限+修正
```
...
| 测试文件 | 用例数 | 内容 |
|---------|--------|------|
| `test_core.py` | 5 | 核心数据结构、轨迹解析、错误定位 |
| `test_judge_executor.py` | 10 | JSON 解析、容错、初始化、降级 |
| `test_process_reward.py` | 2 | 步骤级评分、迭代改善 |
| `test_permissions.py` | 18 | 四级权限、HITL 审批、审计日志 |
| `test_scenarios.py` | 9 场景 | 迭代行为、震荡检测、复杂输入、HITL 拦截 |
| `test_e2e.py` | 10 阶段 | 端到端全流程集成 |
| `test_integration.py` | 10 | 权限包装器、自动触发、工厂函数 |

**共 7 个测试文件、64+ 个测试用例，全部通过。**
