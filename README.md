...
> 覆盖工具调用、记忆、RAG、多 Agent 编排、推理增强、评测 Harness、安全沙箱、Eval 评测管线与 Dashboard 可视化全链路。
...
react_loop 入口（普通或 Worker）:
  ...
  │     不触发: memory.json 为空时。
  │
  ├── step 0: Harness 轨迹记录（手写版 harness/recorder.py + LangGraph 版 graph/harness/recorder.py）
...
# 指定 Provider 运行评测
python -m eval --provider deepseek
...
### Harness 层（harness/ + graph/harness/）
...
- **Sandbox：** 支持白名单（快速工具不走沙箱避免进程开销），自动 fallback 到手写版 runner
- **Replay：** `python -m graph.harness.replay --step` 兼容手写版和 LangGraph 版的轨迹格式
...
| `web_search(query)` | 搜索互联网（DuckDuckGo，手写版；AnySearch，graph 版） | tools/web_search.py |
...
├── eval/               # Agent Eval 评测管线（6 文件）
│   ├── dataset.json   # 26 条测试用例，覆盖 12 个功能模块
│   ├── dataset.py     # 用例加载器，支持按 tag 筛选
│   ├── runner.py      # 批量执行 Agent（subprocess + Recorder 集成）
│   ├── scorer.py      # 4 维评分（工具/内容/步数/答案）
│   ├── report.py      # 报告生成 → eval/reports/ 目录
│   └── __init__.py    # EvalRunner 统一入口 + CLI
├── harness/            # Harness 层（沙箱 + 记录 + 重放）
│   ├── __init__.py     # 统一 Harness 入口
│   ├── recorder.py     # 轨迹记录
│   ├── sandbox.py      # 子进程隔离
│   ├── replay.py       # 离线回放
│   └── _sandbox_runner.py  # 子进程执行脚本
├── mcp_client.py       # MCP 协议客户端
├── rag.py              # RAG 检索增强生成
├── memory.py           # 语义记忆系统
├── eval.py             # [已弃用] 旧版评测，请使用 eval/ 包
...
├── notes/              # 开发笔记（bug 记录/架构设计/Dashboard 心路历程）
├── graph/              # LangChain/LangGraph 全栈版本（与手写版并存）
│   ├── agent.py       # 单 Agent StateGraph（call_model → tools → extract_memory）
│   ├── orchestrator.py# 多 Agent 编排（supervisor → worker → join）
│   ├── llm.py         # ChatOpenAI 封装
│   ├── tools.py       # @tool 装饰器定义
│   ├── rag.py         # FAISS + HuggingFaceEmbeddings
│   ├── memory.py      # 语义记忆系统（BGE + LRU + 语义去重）
│   ├── prompts.py     # 角色模板 + CoT 注入
│   ├── context.py     # 上下文管理
│   ├── mcp.py         # MCP 客户端连接
│   ├── main.py        # CLI 入口（交互 + 单次查询 + Harness 集成）
...
`graph/agent.py` 使用 `StateGraph` 定义 Agent 循环，通过 `config["configurable"]["harness"]` 注入 Harness 实例，节点函数自动记录轨迹：
...
`graph/harness/` 通过 LangGraph 的 `config["configurable"]["harness"]` 透传：
```python
...
| **可观测性** | Harness（`config["configurable"]["harness"]` 注入） | `graph/harness/` | call_model 和 tools_node 自动记录 thought/action/observation |
...
- [x] graph/harness/ LangGraph 版轨迹记录 + 沙箱 + 回放
