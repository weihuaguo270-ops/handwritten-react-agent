...
- **Sandbox（harness/sandbox.py）：** 三策略模式——off(全部直接)/auto(自动按风险判断,默认)/on(全部隔离)。subprocess + timeout 隔离不可信代码，AST 白名单安全解析；`classify_risk()` 对 14 个工具打标签（safe/io/cpu），未知工具默认 safe（MCP/HTTP 不走沙箱）；支持启动时预热缓存
